...
> [!CAUTION]
> Generische AI-Ästhetik zerstört die Premium-Wirkung. Diese Patterns sind **verboten**:
...
.reveal-delay-3 { animation-delay: 350ms; }
.reveal-delay-4 { animation-delay: 500ms; }
.reveal-delay-5 { animation-delay: 700ms; }
```
...
/* ═══════ Sizes ═══════ */
.btn-sm { @apply text-sm px-3.5 py-1.5 rounded-lg; }
.btn-lg { @apply text-lg px-7 py-3.5 rounded-2xl; }
```
...
.form-error-message {
    @apply text-sm mt-1.5 flex items-center gap-1.5;
...
.modal-content {
    @apply max-w-lg w-full mx-4 rounded-2xl overflow-hidden;
...
.modal-header {
    @apply px-7 py-5 flex items-center justify-between;
    border-bottom: 1px solid var(--color-border-subtle);
}
...
.modal-footer {
    @apply px-7 py-4 flex justify-end gap-3;
    background: var(--color-surface-elevated);
...
<section class="relative grid grid-cols-12 gap-0 min-h-[60vh] mesh-bg">
    <!-- Content: Bewusst nicht zentriert, links-lastig -->
    <div class="col-span-7 col-start-2 flex flex-col justify-center py-20
                reveal reveal-delay-1">
        <h1 class="text-5xl font-bold tracking-tight
                   text-[var(--color-text)]">
            TicketsPlease
        </h1>
        <p class="mt-4 text-xl text-[var(--color-text-muted)] max-w-lg">
            Beschreibung hier
        </p>
    </div>
    ...
    <div class="absolute right-0 top-0 w-1/3 h-full
                bg-gradient-to-bl from-[var(--brand-accent)]/10 to-transparent
                -skew-x-6 origin-top-right"
         aria-hidden="true">
    </div>
</section>
```
