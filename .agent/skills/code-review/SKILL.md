...
name: code-review
description:
  Reviews code changes against all TicketsPlease project standards including
  Clean Architecture, DDD, Security, Testing, UI/a11y, and documentation. Use
  when reviewing PRs, checking code quality, or validating changes before
  commit.
---
...
- **B1 – Dependency Direction:** `Domain.Entity` referenziert
  `Infrastructure.DbContext` direkt. → Entity darf keine
  Infrastructure-Abhängigkeit haben.

- **M2 – CancellationToken:** In `GetTicketQueryHandler.Handle()` fehlt `ct` bei
  `ToListAsync()`. → `await _repo.GetAllAsync(cancellationToken)` verwenden.
