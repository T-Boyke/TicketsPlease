...
## Context
In a Code-First approach, ensuring that relationship constraints (Foreign Keys, Delete Behavior) are correctly implemented and enforced is critical. We need a way to verify this without manual database inspection.
...
1.  **Architecture Tests (Statisch)**:
    ...
    - Rule: Every navigation property (e.g., `User AssignedUser`) must have a corresponding ID property (e.g., `Guid? AssignedUserId`).
...
2.  **Integration Tests (Dynamisch)**:
    ...
    - Tests verify that `SaveChangesAsync()` throws exceptions on FK violations and that `Include()` correctly loads related data.
...
- **Negative**: Requires a configured CI/CD environment with Docker support for full MSSQL verification.
- **Neutral**: `AppDbContext` must be provider-agnostic for some configurations (e.g., `RowVersion`).
