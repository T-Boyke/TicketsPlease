...
1. **[.NET 10 SDK](https://dotnet.microsoft.com/download/dotnet/10.0):** Das Herzstück. Ohne das
   SDK läuft kein C# Code.
2. **[Docker Desktop](https://www.docker.com/products/docker-desktop/):** Wir nutzen Docker für die
   SQL-Datenbank, damit du nichts lokal auf deinem Windows installieren musst.
...
4. **Tailwind CSS Build:**
   Damit die Styles korrekt angezeigt werden, muss Tailwind generiert werden. Dies geschieht nun
   **automatisch** beim `dotnet build`, da wir ein lokales `tailwindcss.exe` integriert haben:
...
3. **Login-Test:** Registriere einen neuen Benutzer (Username, Vorname und E-Mail sind Pflicht!) und
   logge dich ein.
...
- **CSS sieht komisch aus?** Führe `dotnet build` erneut aus, um sicherzustellen, dass Tailwind
  alle Klassen generiert hat.
